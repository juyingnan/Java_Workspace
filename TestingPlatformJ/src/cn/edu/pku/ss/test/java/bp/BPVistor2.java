...
//2013-1-21
//Ju Yingnan
public class BPVistor2 extends SingleMethodVisitor<Object, Object>
{
	...
	public Object visit(IfStmt n, Object arg)
	{
		...
		if (isInTargetMethod())
		{
			...
			logic.getGraph().getLast()
					.setBeginLine(n.getThenStmt().getBeginLine());
			...
			if (n.getElseStmt() != null)
			{
				...
				logic.getGraph().getLast()
						.setBeginLine(n.getElseStmt().getBeginLine());
				logic.getGraph().getLast()
						.setEndLine(n.getElseStmt().getEndLine());
				// if(!logic.getGraph().getById(first.getId()).hasNext()){
				// logic.pushNextChild(first);
				// }
			}
	...
	protected ConditionTreeNode visitCondition(Expression expr, boolean isLoop)
	{
		...
		if (expr.getClass() == BinaryExpr.class)
		{
					...
					|| bexp.getOperator() == Operator.or)
			{
				...
				root.setBeginLine(expr.getBeginLine());
				root.setEndLine(expr.getEndLine());
				// 设置左边的节点
			...
			else
			{
				...
				ConditionTreeNode ctNode = new ConditionTreeNode(
						bexp.toString(), id);
				ctNode.setBeginLine(bexp.getBeginLine());
				ctNode.setEndLine(bexp.getEndLine());
				return ctNode;
			}
		}
		...
		else
		{
			...
			int id = flushExpression(expr, isLoop);
			ConditionTreeNode ctNode = new ConditionTreeNode(expr.toString(),
					id);
			ctNode.setBeginLine(expr.getBeginLine());
			ctNode.setEndLine(expr.getEndLine());
			return ctNode;
		}
	...
	protected int flushExpression(Expression expr, boolean isLoop)
	{
		...
		if (logic.isInCondition() == false)
		{
			...
			logic.getGraph().getLast().setBeginLine(expr.getBeginLine());
			logic.getGraph().getLast().setEndLine(expr.getEndLine());
