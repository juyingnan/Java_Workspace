...
public class KWIC_Modified
{
			...
			while (c != -1)
			{
				...
				switch ((byte) c)
				{
					...
					default:
						...
						// if this is a new line we need to update the line
						// index
						if (is_new_line)
						{

							// if the line index array is full, we make a new
							// index
							// array.
							// the length of the new index array is the length
							// of
							// the old
							// index array + 32
							...
							// we assign the index in the original char array as
							// the
							// start of the new line and increment the line
							// counter
							line_index_[line_count] = char_count;
							line_count++;

							// we handled the new line, so we set the new line
							// flag
							// to false
							is_new_line = false;
						...
						if (is_new_word)
						{
							...
							if (is_line_started)
							{
								...
								// if the chars array is full, we make a new
								// chars
								// array.
								// the length of the new chars array is the
								// length
								// of the old
								// index array + 2048
								// at the end we copy the old index array into
								// the
								// new one and work
								// further with the new one
							...
							// we handled the new word, so we set the new word
							// flag
							// to false
							is_new_word = false;
						}
						...
						// array.
						// the length of the new chars array is the length of
						// the
						// old
						// index array + 2048
	...
	public void CircularShift_Modified()
	{
		...
		for (String line : lines)
		{
			...
			for (String word : wordsOfLine)
			{
				...
				shifts_lines_len_[shifts_lines_count++] = shift_line.length();
			}
		}
	...
	public void Alphabetizing_Modified()
	{
	...
	{
		KWIC_Modified kwic = new KWIC_Modified();
		if (args.length != 1)
		{
		...
		kwic.input(args[0]);
		// kwic.circularShift();
		kwic.CircularShift_Modified();
		kwic.filter();
		// kwic.alphabetizing();
		kwic.Alphabetizing_Modified();
		// kwic.output();
		kwic.newOutPut();
	}
